...
(defn get-pusher-factory [async-pusher-enabled]
...
(defn processor [resp thread]
  (timbre/with-merged-config
    ...
    (let [opts (:opts resp)
          {:keys [user-name  credentials mode]}  opts]
      (_case  (result/process resp)
              ...
              result/TOO-MANY-TREADS (do
                                       (debug (format  "Thread %s awaken. Retry action." thread))
                                       tr/RETRY-ACTION)

              result/SERVER-NOT-AVAILABLE (do
                                            (debug (format  "Thread %s awaken. Retry action." thread))
                                            tr/RETRY-ACTION)
