...
(defn task-reader
  ...
  (timbre/with-merged-config
    {:appenders {:println {:enabled? false}
                 :spit (appenders/spit-appender {:fname "log/task_reader.log"})}}
    (jdbc/with-db-transaction [t-con @db]
      (let [lock-time (unixtime->timestamp (tod))
            lock-count ((jdbc/execute!   t-con
                             (format sql-lock-tasks-on-time
                                     lock-time
                                     pusher-id
                                     condition
                                     chunk_size)) 0)]
      (debug pusher-id ":locked " lock-count " items with condition " (format sql-lock-tasks-on-time
                                                 lock-time
                                                 pusher-id
                                                 condition
                                                 chunk_size))
